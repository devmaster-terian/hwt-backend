    ...
    function createSuite(buffered) {
        describe(buffered ? "with buffered rendering" : "without buffered rendering", function() {
            ...
            describe("basic editing", function(){
                ...
                beforeEach(function(){
                    makeGrid();
                    editorParentNode = ((view.getScrollerEl ? view.getScrollerEl() : view.getTargetEl()) || view.getTargetEl()).dom;
                });
                ...
                it('should move editor into the correct side when a column is locked', function() {
                    var ed, field, lockedPlugin, normalPlugin;
                    ...
                    lockedPlugin = grid.lockedGrid.plugins[0];
                    normalPlugin = grid.normalGrid.plugins[0];
                    triggerCellMouseEvent('dblclick', 0, 1);
                    
                    waitsFor(function() {
                        if (normalPlugin.activeEditor && normalPlugin.activeEditor.field.hasFocus) {
                            ed = normalPlugin.activeEditor;
                            field = ed.field;
                            return true;
                        }
                    }, 'edit to start at 0,1');
                    runs(function() {
                        plugin = normalPlugin;
                        triggerEditorKey(27);
                    });
                    ...
                    runs(function() {
                        grid.lock(colRef[1], 0);
                        triggerCellMouseEvent('dblclick', 0, 0);
                    });
                    waitsFor(function() {
                        return ed.isVisible();
                    }, 'editor to reappear at 0,0');

                    // Should have moved to the locked view along with its column
                    runs(function() {
                        expect(ed.el.dom.parentNode).toBe(grid.lockedGrid.view.el.dom);
                    });
                });
            ...
            describe("events", function() {
                ...
                describe("edit", function(){
                    ...
                    it("should focus on the edited cell", function() {
                        spyOn(plugin,'fireEvent');
                        ...
                        triggerCellMouseEvent('dblclick', 0, 0);
                        plugin.getActiveEditor().field.setValue(321);
                        triggerEditorKey(ENTER);
                        
                        // Waits for the edit process to be complete
                        waitsFor(function(){
                            return plugin.fireEvent.mostRecentCall.args[0] === 'edit';
                        });
                        
                        runs(function() {
                            var cell = view.getCell(rec, col);
                            jasmine.waitForFocus(cell);
                            expect(Ext.Element.getActiveElement()).toBe(cell.dom);
                        });
                    });
                });
            ...
            describe("locking", function(){
                ...
                it("should trigger an edit on the locked part", function(){
                    ...
                    // CellEditors should be rendered into the grid view which they are editing, and should scroll along with the view.
                    editorParentNode = ((activeView.getScrollerEl ? activeView.getScrollerEl() : activeView.getTargetEl()) || activeView.getTargetEl()).dom;
                    expect(plugin.getActiveEditor().el.dom.parentNode).toBe(editorParentNode);
                });
            ...
            describe("key handling", function() {
                ...
                it("should move to the next cell when tabbing", function(){
                    triggerCellMouseEvent('dblclick', 0, 0);
                    triggerEditorKey(TAB);
                    expect(plugin.getActiveColumn()).toBe(colRef[1]);
                });
                ...
                it("should move to the next row if at the last cell", function(){
                    ...
                    triggerEditorKey(TAB);
                    expect(plugin.getActiveColumn()).toBe(colRef[0]);
                    expect(plugin.getActiveRecord()).toBe(getRec(1));
                });
            ...
            describe('Autorepeat TAB in ungrouped grid', function() {
                ...
                webkitIt('should not lose track of editing position during repeated tabbing', function() {
                    ...
                    runs(function() {
                        // Tab to 1.5. Will drop out of editing, but the cell should be focused so the NavigationModel picks up the RIGHT
                        jasmine.fireKeyEvent(Ext.Element.getActiveElement(), 'keydown', TAB);
                    });
                    waitsFor(function() {
                        return Ext.Element.getActiveElement() === view.getCellByPosition({row: 0, column: 4}).dom;
                    });
                    runs(function() {
                        // Navigate to 2.1
                        jasmine.fireKeyEvent(Ext.Element.getActiveElement(), 'keydown', TAB);
                    ...
                    runs(function() {
                        // Tab to 2.5. Will drop out of editing, but the cell should be focused so the NavigationModel picks up the RIGHT
                        jasmine.fireKeyEvent(Ext.Element.getActiveElement(), 'keydown', TAB);
                    });
                    waitsFor(function() {
                        return Ext.Element.getActiveElement() === view.getCellByPosition({row: 1, column: 4}).dom;
                    });
                    runs(function() {
                        // Navigate to 3.1
                        jasmine.fireKeyEvent(Ext.Element.getActiveElement(), 'keydown', TAB);
                ...
                webkitIt('should use the same editor for same typed cells, and not blur between edits', function() {
                    ...
                    activeEditor.on({
                        complete: function() {
                            ...
                            editorHidden = !activeEditor.isVisible();
                        }
                    });
                    ...
                    // EXTJSIV-12351 new field would blur a short time after beginning edit
                    waits(100);
                    runs(function() {
                        ...
                        // Active editor is not hidden at the time the edit is completed.
                        // To avoid focus leavinbg the panel, it's hidden only after the new editor takes focus.
                        expect(editorHidden).toBeFalsy();
                        activeEditor = plugin.getActiveEditor();
                        expect(activeEditor.isVisible()).toBe(true);
